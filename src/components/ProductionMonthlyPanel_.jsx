...
function cleanSizes(rows = []) {
  return rows
    .filter((row) => {
      ...
      return size && n !== "unknown" && n !== "unknown size" && Number(row?.production || 0) > 0;
    })
...
async function exportWorkbook({ line, month, data }) {
  ...
  for (const day of data?.days || []) {
    ...
    daily.addRow([
      day.date,
      primarySize(a), Number(a?.production || 0),
      primarySize(b), Number(b?.production || 0),
      primarySize(c), Number(c?.production || 0),
      Number(day.totalProduction || 0),
    ]);
  }

  daily.columns = [14, 14, 22, 14, 22, 14, 22, 20].map((width) => ({ width }));
  [3, 5, 7, 8].forEach((i) => { daily.getColumn(i).numFmt = '#,##0.00'; });
  ...
  bySize.addRow([]);
  bySize.addRow(["Tile Size", "Production (m²)", "Share (%)", "Current Shift (m²)"]).font = { bold: true };
...
export default function ProductionMonthlyPanel({ line, month, data, loading = false }) {
  ...
  const stackedOption = useMemo(() => ({
    ...
    yAxis: { type: "value", name: "Production (m²)", splitLine: { lineStyle: { color: "#eef2f7" } } },
  ...
  const sizeOption = useMemo(() => ({
    ...
    yAxis: { type: "category", inverse: true, data: sizes.map((r) => r.size), axisTick: { show: false } },
    series: [{
      type: "bar",
      barMaxWidth: 22,
      ...
      label: {
        show: true,
        position: "right",
        formatter: (params) => `${fmt(params.data.raw.production)} m² (${fmt(params.data.raw.percentage, 1)}%)`,
        color: "#183153",
        fontWeight: 600,
      },
      data: sizes.map((row) => ({ value: Number(row.production || 0), raw: row })),
    }],
  }), [sizes]);
    ...
    <section className="production-monthly-section">
      <Row gutter={[12, 12]}>
        <Col xs={24} xl={14}>
          ...
          >
            {days.length ? (
              <ReactECharts option={stackedOption} notMerge lazyUpdate style={{ height: 260, width: "100%" }} />
            ) : (
              <div className="production-chart-empty"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No monthly production data" /></div>
            )}
          </Card>
        </Col>
        ...
        <Col xs={24} xl={10}>
          ...
          >
            {sizes.length ? (
              <ReactECharts option={sizeOption} notMerge lazyUpdate style={{ height: 260, width: "100%" }} />
            ) : (
              <div className="production-chart-empty"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No size data" /></div>
            )}
            <div className="production-month-total">Total: {fmt(data?.totalProduction)} m²</div>
          </Card>
        </Col>
      </Row>
    </section>
